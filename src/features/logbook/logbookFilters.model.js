...
export function normalizeLogbookFilters(savedFilters, bounds = DEFAULT_DISTANCE_BOUNDS) {
  const nextFilters = {
    ...
    origin: toSelectionArray(savedFilters?.origin).map((value) => value.toUpperCase()),
    destination: toSelectionArray(savedFilters?.destination).map((value) => value.toUpperCase()),
