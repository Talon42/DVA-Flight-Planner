...
export function normalizeLogbookFilters(savedFilters, bounds = DEFAULT_DISTANCE_BOUNDS) {
  const nextFilters = {
    ...
    departure: toSelectionArray(savedFilters?.departure ?? savedFilters?.origin).map((value) => value.toUpperCase()),
    arrival: toSelectionArray(savedFilters?.arrival ?? savedFilters?.destination).map((value) => value.toUpperCase()),
