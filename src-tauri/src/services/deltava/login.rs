...
use crate::services::deltava::auth::DeltaVirtualAuthContext;
...
(() => {
  ...
  const submittedKey = `flightPlannerDeltaLoginSubmitted:${nonce}`;
  const pendingPasswordKey = `flightPlannerDeltaLoginPendingPassword:${nonce}`;
  const failedKey = `flightPlannerDeltaLoginFailed:${nonce}`;
  ...
  const clearAllSessionValues = () => {
    clearSessionValues();
    try {
      window.sessionStorage.removeItem(failedKey);
    } catch (_) {}
  };
  ...
  const getNormalizedPageText = () =>
    String(document.body?.innerText || document.body?.textContent || '')
      .replace(/\s+/g, ' ')
      .trim();

  const detectDvaLoginFailure = () => {
    const pageText = getNormalizedPageText();
    const lowerText = pageText.toLowerCase();

    if (!lowerText.includes('login failure')) {
      return '';
    }

    if (lowerText.includes('unknown user name')) {
      return 'Delta Virtual rejected the login credentials.';
    }

    if (lowerText.includes('invalid credentials') || lowerText.includes('cannot authenticate')) {
      return 'Delta Virtual rejected the login credentials.';
    }
  ...
  const maybeReportFailure = () => {
    ...
    if (getSessionValue(failedKey) === '1') {
      return true;
    }

    const dvaFailureReason = detectDvaLoginFailure();
    if (dvaFailureReason) {
      emitDebug('auth:failed:dva-login-failure');
      setSessionValue(failedKey, '1');
      postAuthMessage({
        kind: 'loginFailed',
        reason: dvaFailureReason
      });
      clearSessionValues();
      return true;
    }
    ...
    emitDebug('auth:failed:login-error');
    setSessionValue(failedKey, '1');
    postAuthMessage({
      kind: 'loginFailed',
      reason: 'Delta Virtual rejected the login credentials.'
    });
    clearAllSessionValues();
    return true;
  };
