...
use crate::{append_sync_log_debug, DeltaSyncPayload, DELTAVA_SYNC_DOWNLOAD_FILE};
...
use super::{
    ...
    profile_cache,
    sync_types::{
        DeltaWebLogbookRefreshResult, DeltaWebSyncResult,
...
fn summarize_warnings(warnings: &[String]) -> Option<String> {
    ...
    let preview = warnings.iter().take(3).cloned().collect::<Vec<_>>().join("; ");
...
pub(crate) async fn build_delta_sync_payload_from_web_result(
    ...
    let xml_text = if result.xml.ok {
        ...
        warnings.push(result.xml.error.unwrap_or_else(|| "Delta Virtual schedule XML download failed.".into()));
        None
    };
    ...
    let logbook_json = if result.logbook.ok {
        ...
        if json_text.len() > MAX_DELTAVA_LOGBOOK_JSON_BYTES {
            ...
            match store_logbook_json(app, &json_text, result.logbook.content_type).await {
                ...
                Err(error) => { warnings.push(error); None }
            }
        }
    } else {
        warnings.push(result.logbook.error.unwrap_or_else(|| "Delta Virtual logbook JSON download failed.".into()));
        None
    };

    if logbook_json.is_some() {
        let _ = profile_cache::resolve(app, result.logbook.export_id.as_deref(), true).await;
    }
    ...
    let accomplishment_eligibility = if let Some(accomplishments) = result.accomplishments {
        if accomplishments.ok {
            ...
            if html_text.len() > MAX_DELTAVA_ACCOMPLISHMENT_HTML_BYTES {
                warnings.push(format_too_large_error("accomplishment eligibility HTML", MAX_DELTAVA_ACCOMPLISHMENT_HTML_BYTES));
                None
            } else {
                let parsed = parse_accomplishment_eligibility_html(&html_text);
                append_sync_log_debug(debug_enabled, &format!("accomplishments:parsed rows={}", parsed.rows.len()));
            ...
            warnings.push(accomplishments.error.unwrap_or_else(|| "Delta Virtual accomplishment eligibility download failed.".into()));
            None
        }
    } else {
        None
    ...
    let xml_status = if xml_text.is_some() { "success" } else { "failed" }.to_string();
    let logbook_status = if logbook_json.is_some() { "success" } else { "failed" }.to_string();
    if xml_text.is_none() && logbook_json.is_none() {
        return Err(format!(
            "download_failed: Delta Virtual sync failed. {}",
            summarize_warnings(&warnings).unwrap_or_else(|| "No sync artifacts were downloaded.".into())
        ));
    }

    Ok(DeltaSyncPayload {
        file_name: xml_text.as_ref().map(|_| DELTAVA_SYNC_DOWNLOAD_FILE.to_string()),
        xml_text,
        status: if xml_status == "success" && logbook_status == "success" { "success" } else { "partial" }.to_string(),
        xml_status,
        logbook_status,
...
pub(crate) async fn build_delta_logbook_refresh_payload_from_web_result(
    ...
    let logbook_json = if result.logbook.ok {
        ...
        warnings.push(result.logbook.error.unwrap_or_else(|| "Delta Virtual logbook refresh failed.".into()));
        None
    };
    ...
    let Some(logbook_json) = logbook_json else {
        return Err(format!(
            ...
            summarize_warnings(&warnings).unwrap_or_else(|| "No logbook artifact was downloaded.".into())
        ));
    };
    let _ = profile_cache::resolve(app, result.logbook.export_id.as_deref(), true).await;
