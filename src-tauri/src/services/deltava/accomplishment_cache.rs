...
pub(crate) fn read(app: &AppHandle) -> DeltaAccomplishmentEligibilityStore {
    let Ok(path) = crate::app::paths::build_accomplishment_eligibility_path(app) else { return DeltaAccomplishmentEligibilityStore::default(); };
    let Ok(text) = fs::read_to_string(path) else { return DeltaAccomplishmentEligibilityStore::default(); };
    serde_json::from_str(&text).unwrap_or_default()
}

pub(crate) fn store(app: &AppHandle, value: &DeltaAccomplishmentEligibilityStore) -> Result<DeltaAccomplishmentEligibilityStore, String> {
    let path = crate::app::paths::build_accomplishment_eligibility_path(app)?;
    let json = serde_json::to_string_pretty(value).map_err(|error| format!("download_failed: Unable to serialize accomplishment eligibility: {error}"))?;
    crate::services::storage::atomic_json::write_atomic_json(&path, &json).map_err(|error| format!("download_failed: Unable to write accomplishment eligibility: {error}"))?;
    Ok(value.clone())
}
...
mod tests {
    ...
    fn failed_atomic_accomplishment_replacement_preserves_last_good_cache() {
        ...
        let directory = std::env::temp_dir().join(format!("dva-accomplishment-cache-{}-{unique}", std::process::id()));
