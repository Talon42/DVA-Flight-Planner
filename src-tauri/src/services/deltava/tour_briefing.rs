use crate::services::deltava::auth::read_auth_context_internal;
...
fn validate_briefing_url(raw_url: &str) -> Result<String, String> {
    ...
    let url = if normalized.starts_with("/attach/tbrief/") {
        Url::parse(&format!("https://www.deltava.org{normalized}")).map_err(|error| {
            format!("download_failed: Invalid Delta Virtual briefing URL: {error}")
        })?
    } else {
    ...
    let allowed_host = url
        .host_str()
        .map(|host| {
            host.eq_ignore_ascii_case("deltava.org") || host.eq_ignore_ascii_case("www.deltava.org")
        })
        .unwrap_or(false);

    if url.scheme() != "https" || !allowed_host || !url.path().starts_with("/attach/tbrief/") {
...
async fn fetch_delta_virtual_tour_briefing_windows(
    ...
    .build()
    .map_err(|error| {
        format!("download_failed: Unable to open Delta Virtual briefing window: {error}")
    })?;
