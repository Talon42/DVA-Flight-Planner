...
use crate::{
    append_sync_log, services::deltava::draft::DVA_DRAFT_WEBVIEW_DIR, DELTAVA_SYNC_DOWNLOAD_FILE,
};
    ...
    if let Ok(entries) = fs::read_dir(directory) {
        for entry in entries.flatten() {
            ...
            let is_legacy = path
                ...
                .map(|name| {
                    name.starts_with("deltava-pfpxsched-")
                        && name.to_ascii_lowercase().ends_with(".xml")
                })
                .unwrap_or(false);
            if path.is_file() && is_legacy {
...
fn prune_webview_profile(root: &Path) {
    ...
    for name in WEBVIEW_ROOT_PRUNE_DIRS {
        remove_path_if_exists(&root.join(name));
    }
    for name in WEBVIEW_ROOT_PRUNE_FILES {
        remove_path_if_exists(&root.join(name));
    }
    ...
    for name in WEBVIEW_PROFILE_PRUNE_DIRS {
        remove_path_if_exists(&default_profile.join(name));
    }
    for name in WEBVIEW_PROFILE_PRUNE_FILES {
        remove_path_if_exists(&default_profile.join(name));
    }
}
