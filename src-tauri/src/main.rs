...
async fn build_delta_tours_sync_payload_from_web_result(
    app: &AppHandle,
                    ...
                    .add_WebMessageReceived(
                        &WebMessageReceivedEventHandler::create(Box::new(move |_, args| {
                            ...
                            if let Some(payload_text) = message.strip_prefix(DELTAVA_TOURS_SYNC_RESULT_MESSAGE_PREFIX) {
                                ...
                                tauri::async_runtime::spawn(async move {
                                    let result = match serde_json::from_str::<DeltaToursSyncResultEnvelope>(&payload_text) {
                                        Ok(web_result) if web_result.nonce == sync_nonce && web_result.payload.source == "dva" => {
                                            build_delta_tours_sync_payload_from_web_result(&app_handle, web_result.payload).await
                                        }
                                        Ok(_) => return,
                                        Err(error) => Err(format!(
                                    ...
                                    match result {
                                        Ok(payload) => {
                                            append_sync_log(if payload.ok { "succeeded" } else { "failed" });
                                            app_handle
                                                .state::<DeltaToursSyncManager>()
                                                .finish(DELTAVA_TOURS_SYNC_LABEL, Ok(payload));
                                        }
                                        Err(error) => {
                                            append_sync_log("failed");
                                            app_handle
                                                .state::<DeltaToursSyncManager>()
                                                .finish(DELTAVA_TOURS_SYNC_LABEL, Err(error));
                                        }
                                    }
